...
In this project, I decided to discard ***pregnancy***, ***contact_other_covid***, ***covid_res***, ***icu*** and ***intubed***. These variables has significant portion of missing value and we don't know that these are missing completely at random (MCAR) or missing not at random (MNAR), it is the best that we should discard these variable to avoid parameters bias. Data processing is provided below.
...
ident <- function(x){
  temp = case_when(
    x == 1 ~ 1, 
    x == 2 ~ 0,
    TRUE ~ 100 #100 is the flag for NULL
  )
  return(temp)
}
...
Next, we use values from ***date_symptoms*** and ***entry_date*** to create a new variable ***duration***. We define ***duration*** as time (day) between the day the symptoms start showing and the day the patient goes to the hospital to check up.
...
Now we discard variables that we are not going to use.
...
Then we drop all NULL record.

```{r}
#deleting NULL record
temp = data %>% filter_all(any_vars(. %in% 100)) #select every record with NULL
drop_id = temp$id #get id of NULL records
data = data[-drop_id, ] #delete records with NULL
...
***II. Fitting the logistic regression model and interpretation***

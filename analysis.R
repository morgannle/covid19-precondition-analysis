library(dplyr)
library(plotly)

wd ="C:/Users/nghia/OneDrive/Documents/GitHub/covid19-precondition-analysis"
setwd(wd)

source("cfold.R")

data = as.data.frame(
  read.csv("covid.csv")
)

data$id = NULL

subset = data[ ,c(3,4,5,8)]
data = data[ , -c(3,4,5,8)]
data = apply(data, 2, as.numeric)

#this function assign 1 as yes and 0 as no
ident <- function(x){
  temp = case_when(
    x == 1 ~ 1, 
    x == 2 ~ 0,
    TRUE ~ 100 #100 is the flag for NULL record
  )
  return(temp)
}

#convert variables to factor
data = as.data.frame(
  apply(data, 2, ident)
  )

#convert age to factor
subset = subset %>% mutate(
  age_factor = case_when(
    (0 < age & age < 20) ~ "0 - 20",
    (20 <= age & age < 40) ~ "20 - 40",
    (40 <= age & age < 60) ~ "40 - 60",
    (60 <= age & age < 80) ~ "60 - 80",
    (80 <= age & age < 100) ~ "80 - 100",
    TRUE ~ "100+"
    )
  )

#death flag
subset = subset %>% mutate(
  subset,
  death = ifelse(date_died == "9999-99-99", 0, 1))
#time from symptoms to entry
subset$date_symptoms = as.Date(subset$date_symptoms, 
                                format = "%d-%m-%Y")
subset$entry_date = as.Date(subset$entry_date, 
                          format = "%d-%m-%Y")
subset$duration = as.numeric(subset$entry_date - subset$date_symptoms) 

subset = subset %>% mutate(
  symptoms_to_entry = case_when(
    (0 < duration & duration < 5) ~ "0 - 5",
    (5 <= duration & duration < 10) ~ "5 - 10",
    TRUE ~ "10+"
    )
  )

data$id = c(1:nrow(data)) #create id
data = cbind(data, subset) #combine data sets

#deleting unsed variables
data$count = NULL 
data$date_died = NULL
data$date_symptoms = NULL
data$entry_date = NULL
data$age = NULL
data$pregnancy = NULL
data$contact_other_covid = NULL
data$covid_res = NULL
data$patient_type = NULL
data$duration = NULL

#deleting NULL record
temp = data %>% filter_all(any_vars(. %in% 100)) #select every record with NULL
id = temp$id
data = data[-id, ] #delete record with NULL
data$id = NULL #discard id
temp$id = NULL

#convert variables to factor
data$age_factor = factor(data$age_factor,
                         levels = c("0 - 20", 
                                    "20 - 40", 
                                    "40 - 60", 
                                    "60 - 80",
                                    "80 - 100",
                                    "100+"))
data$symptoms_to_entry = factor(data$symptoms_to_entry,
                                levels = c("0 - 5",
                                           "5 - 10",
                                           "10+"))
for(i in 1:17){
  if(i == 15 | i == 17) next #skip age_factor variable
  data[ ,i] = factor(data[ ,i],
                     levels = c("0", "1"))
}
data$death = NULL
#shuffle data set
data = data[complete.cases(data), ]
data = data[sample(nrow(data)), ]
train = data[c(1:floor(0.7*nrow(data))), ]
test = data[-c(1:floor(0.7*nrow(data))), ]

impute_model = glm(data = data,
                   formula = icu~.,
                   family = binomial(link = "logit"))
test$prob = predict(impute_model, test, type="response")

test = test %>% mutate(icu_predict = ifelse(prob >= 0.5, 1, 0))
MLmetrics::F1_Score(test$icu, test$icu_predict)